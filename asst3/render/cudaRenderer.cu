...
// 逐像素进行渲染
__global__ void kernelRenderPiexl() {
    ...
    // if (x_pos >= imageWidth || y_pos >= imageHeight) {
    //     return;
    // }
    ...
    float2 pixelCenterNorm = make_float2(invWidth * (static_cast<float>(x_pos) + 0.5f),
                                        invHeight * (static_cast<float>(y_pos) + 0.5f));

    float4* imgPtr = (float4*)(&cuConstRendererParams.imageData[4 * (y_pos * imageWidth + x_pos)]);
...
CudaRenderer::render() {
    ...
    dim3 gridDim((image->width + blockDim.x - 1) / blockDim.x,
                 (image->height + blockDim.y - 1) / blockDim.y);
    kernelRenderPiexl<<<gridDim, blockDim>>>();
    cudaDeviceSynchronize();
}
