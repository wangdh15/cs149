...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double startExecTime = CycleTimer::currentSeconds();
    // run CUDA kernel. (notice the <<< >>> brackets indicating a CUDA
    // kernel launch) Execution on the GPU occurs here.
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);

    // return before all work id done.
    cudaDeviceSynchronize();

    double endExecTime = CycleTimer::currentSeconds();
    //
    ...
    double ExecTime = endExecTime - startExecTime;
    printf("Execute Time: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * ExecTime, GBPerSec(totalBytes, ExecTime));
